...
class HeaderStack extends Component {
  ...
  _buildHeaders = (lastIndex = 0, nextIndex = 0) => {
    this.renderStack = this.routes.map((route, index) => {
      ...
      let previousSceneProps;
      if (index > 0) {
        const { component: PreviousComponent, ...sceneProps} = this.routes[index - 1]; // eslint-disable-line
        previousSceneProps = sceneProps;
      }
      ...
      return (
        ...
        >
          <Header key={index} navigation={this.props.navigation} sceneProps={sceneProps} previousSceneProps={previousSceneProps} />
        </AnimatedView>
      );
    });
  }
  render() {
    ...
    let animationName;
    // What a mess this is...
    /* if (this.isFirstRender && currentRoute.hideNavBar) {
      return null;
    } else if (currentRoute.hideNavBar && this.lastRouteHideNavBar) {
      return null;
    } else if (this.isPush && currentRoute.hideNavBar) {
