...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case actions.POP_TO: {
      ...
      let navRoot = state.get(currentRootKey);
      const nextKeyIndex = navRoot.get('routes').findIndex((route) => {
        const routeKey = route.get('key').split('-')[0];
        return routeKey === action.key;
      });

      if (nextKeyIndex < 0) {
        throw new Error(`ReactNavigationHelm: Could not find scene ${action.key}`);
      }

      navRoot = navRoot.set('index', nextKeyIndex);
      navRoot = navRoot.update('routes', (routes) => {
        let updatedRoutes = routes.slice(0, nextKeyIndex + 1);
        return updatedRoutes.update(navRoot.get('index'), route => route.merge(action.props));
      });
...
export default onNavigate => (state, action) => {
  ...
  newState = setActiveKey(newState);
